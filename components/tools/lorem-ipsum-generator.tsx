...
const WORD_POOLS: Record<LoremMode, string[]> = {
    ...
    dev: [
        ...
        'feature-flag', 'canary-release', 'blue-green', 'rollback', 'on-call',
        'postmortem', 'runbook', 'SLO', 'SLA', 'error-budget', 'tech-debt',
        'circuit-breaker', 'rate-limiting', 'load-balancing', 'health-check',
        'service-discovery', 'api-gateway', 'reverse-proxy', 'CDN', 'edge-computing',
        'distributed-tracing', 'service-mesh', 'eventual-consistency', 'CQRS',
        'event-sourcing', 'saga-pattern', 'DDD', 'hexagonal-architecture', 'SOLID',
        'zero-downtime', 'graceful-shutdown', 'readiness-probe', 'liveness-probe',
        'semantic-versioning', 'hotfix', 'release-train', 'monorepo', 'trunk-based',
        'TDD', 'BDD', 'DRY', 'YAGNI', 'KISS', 'twelve-factor', 'cloud-native',
        'immutable-infrastructure', 'GitOps', 'pair-programming', 'mob-programming',
    ],
    cyber: [
        ...
        'firewall', 'proxy', 'revocation', 'pinning',
        ...
        'zero-trust', 'ZTNA', 'MFA', 'SSO', 'microsegmentation', 'DMZ',
        'threat-modeling', 'risk-assessment', 'SOC2', 'ISO27001',
        'vulnerability-scan', 'DAST', 'SAST', 'bug-bounty', 'UEBA',
        'anomaly-detection', 'behavioral-analysis', 'incident-response', 'tabletop',
        'log-aggregation', 'honeypot', 'deception', 'canary-token', 'sinkhole',
        'DLP', 'CASB', 'WAF', 'IDS', 'IPS', 'NDR', 'MDR', 'MSSP',
        'spearphishing', 'watering-hole', 'credential-stuffing', 'brute-force',
        'password-spray', 'Kerberoasting', 'pass-the-hash', 'token-theft',
        'living-off-the-land', 'fileless-malware', 'stageless-payload', 'dropper',
    ],
}
...
const DEV_TITLES = [
    'Architecture Review', 'System Design Specification', 'Technical Overview',
    'API Documentation', 'Infrastructure Blueprint', 'Deployment Strategy',
    'Observability Framework', 'Service Mesh Architecture',
    'CI/CD Pipeline Design', 'Database Optimization Report',
    'Scalability Assessment', 'Migration Runbook',
]

const CYBER_TITLES = [
    'Threat Intelligence Report', 'Incident Response Summary',
    'Vulnerability Assessment', 'Red Team Exercise Report',
    'Threat Actor Attribution', 'Security Posture Review',
    'Adversary Simulation Results', 'ATT&CK Coverage Analysis',
    'Detection Engineering Report', 'Purple Team Assessment',
...
function generateParagraph(
    ...
    const count = Math.floor(seededRandom() * (maxSentences - minSentences + 1)) + minSentences
    const sentences: string[] = []
    for (let i = 0; i < count; i++) sentences.push(generateSentence(pool, wMin, wMax))
...
function genApiTable(pool: string[]): Block {
    ...
    return { kind: 'table', headers: ['Endpoint', 'Method', 'Description', 'Auth'], rows }
}
...
function genIocTable(): Block {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (type === 'IP') {
            ...
            indicator = Array.from({ length: 16 }, () => Math.floor(seededRandom() * 16).toString(16)).join('') + '...'
        } else if (type === 'Domain') {
            indicator = Array.from({ length: 8 }, () => alpha[Math.floor(seededRandom() * 26)]).join('') + '.com'
        } else if (type === 'URL') {
            indicator = 'https://' + Array.from({ length: 8 }, () => alpha[Math.floor(seededRandom() * 26)]).join('') + '.net/payload'
        } else {
            indicator = 'HKLM\\Software\\' + Array.from({ length: 8 }, () => alpha[Math.floor(seededRandom() * 26)].toUpperCase()).join('')
        }
        rows.push([type, indicator, confs[Math.floor(seededRandom() * confs.length)], statuses[Math.floor(seededRandom() * statuses.length)]])
    }
    return { kind: 'table', headers: ['Type', 'Indicator', 'Confidence', 'Status'], rows }
}
...
function genAttackTable(): Block {
    ...
    const techniques = [
        'Phishing', 'LOLBin Abuse', 'Scheduled Task', 'WMI Execution',
        'Registry Modification', 'Token Impersonation', 'Pass-the-Hash',
        'Remote Services', 'Data Staging', 'C2 Protocol',
    ]
    ...
    return { kind: 'table', headers: ['Tactic', 'Tech ID', 'Technique', 'Detected'], rows }
}
...
function generateStructuredDoc(mode: LoremMode, sectionCount: number): Block[] {
    ...
    if (mode === 'classic') {
        ...
        blocks.push({ kind: 'p', text: MODE_OPENINGS.classic + ' ' + generateParagraph(pool, 1, 3) })
        for (let i = 0; i < sectionCount; i++) {
            ...
            if (seededRandom() > 0.4) {
                ...
                blocks.push({
                    kind: 'ul',
                    items: Array.from({ length: itemCount }, () => generateSentence(pool, 5, 10)),
                })
            }
        }
    } else if (mode === 'dev') {
        const titleBase = DEV_TITLES[Math.floor(seededRandom() * DEV_TITLES.length)]
        blocks.push({ kind: 'h1', text: `${titleBase}: ${cap(generateWords(pool, 2))}` })
        blocks.push({ kind: 'p', text: MODE_OPENINGS.dev + ' ' + generateParagraph(pool, 1, 2, wMin, wMax) })
        for (let i = 0; i < sectionCount; i++) {
            const section = DEV_SECTIONS[i % DEV_SECTIONS.length]
            blocks.push({ kind: 'h2', text: section.heading })
            blocks.push({ kind: 'p', text: generateParagraph(pool, 2, 4, wMin, wMax) })
            if (section.extra === 'api-table') {
                ...
                blocks.push({
                    kind: 'ol',
                    items: Array.from({ length: 3 + Math.floor(seededRandom() * 4) }, () => generateSentence(pool, 6, 12)),
                })
            } else if (section.extra === 'ul') {
                blocks.push({
                    kind: 'ul',
                    items: Array.from({ length: 3 + Math.floor(seededRandom() * 4) }, () => generateSentence(pool, 4, 8)),
                })
            }
        }
    } else {
        const titleBase = CYBER_TITLES[Math.floor(seededRandom() * CYBER_TITLES.length)]
        blocks.push({ kind: 'h1', text: `${titleBase}: ${cap(generateWords(pool, 2))}` })
        blocks.push({ kind: 'p', text: MODE_OPENINGS.cyber + ' ' + generateParagraph(pool, 1, 2, wMin, wMax) })
        for (let i = 0; i < sectionCount; i++) {
            const section = CYBER_SECTIONS[i % CYBER_SECTIONS.length]
            blocks.push({ kind: 'h2', text: section.heading })
            if (section.content === 'p') {
                blocks.push({ kind: 'p', text: generateParagraph(pool, 2, 5, wMin, wMax) })
            } else if (section.content === 'p+ul') {
                blocks.push({ kind: 'p', text: generateParagraph(pool, 1, 3, wMin, wMax) })
                blocks.push({
                    kind: 'ul',
                    items: Array.from({ length: 3 + Math.floor(seededRandom() * 5) }, () => generateSentence(pool, wMin, wMax)),
                })
            } else if (section.content === 'ioc-table') {
                blocks.push(genIocTable())
            } else if (section.content === 'p+ol') {
                blocks.push({ kind: 'p', text: generateParagraph(pool, 1, 2, wMin, wMax) })
                blocks.push({
                    kind: 'ol',
                    items: Array.from({ length: 4 + Math.floor(seededRandom() * 4) }, () => generateSentence(pool, 6, 14)),
                })
...
function blocksToMarkdown(blocks: Block[]): string {
    return blocks.map(b => {
        ...
        if (b.kind === 'ol') return b.items.map((i, idx) => `${idx + 1}. ${i}`).join('\n')
    ...
    return blocks.map(b => {
        ...
        if (b.kind === 'table') {
            const colW = b.headers.map((h, i) =>
                Math.max(h.length, ...b.rows.map(r => (r[i] ?? '').length))
            )
            const pad = (s: string, w: number) => s.padEnd(w)
            return [
                b.headers.map((h, i) => pad(h, colW[i])).join('  '),
                colW.map(w => '-'.repeat(w)).join('  '),
                ...b.rows.map(r => r.map((c, i) => pad(c, colW[i])).join('  ')),
            ].join('\n')
        }
        return ''
    }).join('\n\n')
...
function renderBlocks(blocks: Block[]) {
    return blocks.map((block, i) => {
        if (block.kind === 'h1')
            return <h1 key={i} className="text-2xl font-bold">{block.text}</h1>
        if (block.kind === 'h2')
            return <h2 key={i} className="text-base font-semibold mt-4 pb-1 border-b">{block.text}</h2>
        if (block.kind === 'p')
        ...
        if (block.kind === 'code')
            return (
                <pre key={i} className="bg-muted rounded-md p-3 text-xs font-mono overflow-x-auto">
                        ...
                        <thead>
                            <tr className="border-b">
                                {block.headers.map((h, j) => (
                                    <th key={j} className="text-left py-1.5 px-2 font-semibold whitespace-nowrap">{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {block.rows.map((row, j) => (
                                <tr key={j} className="border-b border-border/40">
                                    {row.map((cell, k) => (
                                        <td key={k} className="py-1.5 px-2 font-mono break-all">{cell}</td>
                                    ))}
                                </tr>
                            ))}
                        </tbody>
...
export default function LoremIpsumGenerator() {
    ...
    const generate = () => {
        ...
        if (structured) {
            ...
            setBlocks(docBlocks)
            setOutput(markdown ? blocksToMarkdown(docBlocks) : blocksToPlain(docBlocks))
            return
        }
        ...
        if (type === 'words') {
            ...
            const sentences: string[] = []
            for (let i = 0; i < n; i++) sentences.push(generateSentence(pool, wMin, wMax))
            if (startWithOpening && sentences.length > 0) sentences[0] = opening
            setOutput(sentences.join(' '))
        } else {
            const paragraphs: string[] = []
            for (let i = 0; i < n; i++) paragraphs.push(generateParagraph(pool, 3, 6, wMin, wMax))
                        ...
                        {!structured && (
                            <label className="flex cursor-pointer items-center gap-2 text-sm select-none">
                                <input
                                    ...
                                    onChange={(e) => setStartWithOpening(e.target.checked)}
